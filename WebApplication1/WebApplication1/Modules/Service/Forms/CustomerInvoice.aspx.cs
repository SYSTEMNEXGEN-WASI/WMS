...
namespace DXBMS.Modules.Service
{
    public partial class CustomerInvoice : System.Web.UI.Page
    {
        ...
        Double Total = 0.0,totaltax=0,totalAmtwithTax=0, totalCustax = 0, totalCusAmtwithTax = 0,InsPart=0;
        DataTable JobDT, LubDT, PartsDT, SubletDT, BoutDT, dt,CusTax;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (Session["TransCode"] != null)
                {
                    ...
                    ddlInvoiceNo.SelectedValue = leadId;

                    LoadReportData();

                    LoadMasterData("InvoiceNo", leadId);
        ...
        private void InsuranceInvoiceEntry()
        {
           ...
           /*40*/ new SqlParameter("@ReceiptMode",SqlDbType.VarChar)
            ...
            IINV_param[40].Value = ddlReciptMode.SelectedValue;
            #endregion
            try
            {
                if (ObjTrans.BeginTransaction(ref Trans) == true)
                {
                    ...
                    if (myFunc.ExecuteSP_NonQuery("[sp_W2_CustomerInvoice_Insert]", IINV_param, Trans))
                    {
                        ...
                        if (myFunc.ExecuteQuery(IQuery, Trans))
                        {
                            ...
                            if (double.Parse(txtGSTAmount.Text.Trim()) != 0)
                            {
                                SaveItemSalesTaxIns(Session["GSTInvNo"].ToString(), strAutoInvoiceNo,Trans);
                            }
                        ...
                        ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "ClientScript", "alert('Record Saved,Updated Successfully Invoice No: " + strAutoInvoiceNo + "')", true);
           ...
           /*38*/ new SqlParameter("@TRNumber",SqlDbType.Char,15),            /*39*/ new SqlParameter("@TRDate",SqlDbType.DateTime),
            ...
            try
            {
                if (ObjTrans.BeginTransaction(ref Trans) == true)
                {
                            ...
                            if (myFunc.ExecuteQuery(IQuery, Trans))
                            {
                                ...
                                if (double.Parse(txtGSTAmount.Text.Trim()) != 0)
                                {
                                    ...
                                    if (myFunc.ExecuteSP_NonQuery("[sp_W2_GSTInvoice_Insert]", GSTINV_param, Trans))
                                    {
                                        ...
                                        SaveItemSalesTax(GSTInvNo, NewInvNo, Trans);
                                    }
                    ...
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "ClientScript", "alert('Record Saved,Updated Successfully Invoice No: " + NewInvNo + "')", true);
                                ...
                                if (double.Parse(txtGSTAmount.Text.Trim()) != 0)
                                {
                                    ...
                                    if (myFunc.ExecuteSP_NonQuery("[sp_W2_GSTInvoice_Insert]", GSTINV_param, Trans))
                                    {
                                        IQuery = "Update CustomerInvoice set SaleInvoice='" + GSTInvNo + "' " +
                                                 "Where DealerCode='" + Session["DealerCode"].ToString() + "' and JobCardCode='" + txtJobCardCode.Text + "' and InvoiceNo = '" + NewInvNo + "'";
                                        myFunc.ExecuteQuery(IQuery, Trans);
                                        SaveItemSalesTaxDep(GSTInvNo, NewInvNo,Trans);
                                    }
        ...
        MDNINV_param[0].Value = Session["DealerCode"].ToString();
            ...
            {
                MDNINV_param[1].Value = ddlInvoiceNo.SelectedValue;
                MDNINV_param[2].Value = strJobCardCode;
                MDNINV_param[3].Value =null ;
            }
            else
            {
                MDNINV_param[1].Value = strJobCardCode;
                MDNINV_param[2].Value = "";
                MDNINV_param[3].Value = null;
            }
        ...
        protected void LoadMasterData(string strClearCondition,string jobcard)
        {
            ...
            if (dsJobCardMaster.Tables[0].Rows.Count > 0)
            {
                ...
                if (strClearCondition == "InvoiceNo")
                {
                    ...
                    ddlReciptMode.SelectedValue = dsJobCardMaster.Tables[0].Rows[0]["ReceiptMode"].ToString();
                }
                else
                {
                ...
                {
                    taxAmount = taxAmount + CustomCDBL((CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["GSTAmount"]) +
                                               CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["FGSTAmount"].ToString()) +
                                               CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["ExTaxAmount"].ToString())));
                    FGSTAmount = FGSTAmount + CustomCDBL(CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["FGSTAmount"]));
                    FTAmount = FTAmount + CustomCDBL(CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["ExTaxAmount"]));
               ...
               double ttx = Convert.ToDouble(txtTaxAmount.Text) ;
                ...
                if (txtJobCardType.Text == "INSURANCE")
                {
                    ...
                    double totalAmtCustomer=0;
                    for (int i = 0; i < CusTax.Rows.Count; i++)
                    {
                        totalAmtCustomer = totalAmtCustomer + CustomCDBL((Convert.ToDecimal(CusTax.Rows[i]["DepAmount"]))) ;
                                               

                    }
                    txtDepAmount.Text = totalAmtCustomer.ToString();
                ...
                if (txtJobCardType.Text == "INSURANCE" && strClearCondition == "JobCard")
                {
                   ...
                   double cusTaxAmt=0;
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        tax = CustomCDBL((Convert.ToDecimal(dsTaxDetail.Tables[0].Rows[i]["GSTAmount"]) +
                    ...
                    totalAmtCustomer = Math.Round(Convert.ToDouble(txtDepAmount.Text)+ cusTaxAmt,2);

                    txtTotalAmtCustomer.Text = totalAmtCustomer.ToString();
                    object InsParts = dsTaxDetail.Tables[0].Compute("Sum(DepAmount)", string.Empty);

                    double temp = InsParts == DBNull.Value ? 0 : Convert.ToDouble(InsParts);

                    txtTotAmtInsComp.Text = CustomCDBL((temp)+
                                             (txtJobsTotal.Text == "" ? 0 : double.Parse(txtJobsTotal.Text))+
                                             (txtPSTAmount.Text == "" ? 0 : double.Parse(txtPSTAmount.Text)) +
                                             (txtGSTAmount.Text == "" ? 0 : double.Parse(txtGSTAmount.Text)) +
            ...
            try
            {
                if (dsTaxDetail.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < drr.Length; i++)
                    {
                            ...
                            Dep = CustomCDBL(100- CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["Dep"].ToString())).ToString();
                            depamt = CustomCDBL((Convert.ToDouble(dsTaxDetail.Tables[0].Rows[i]["Price"].ToString() == "" ? 0 : double.Parse(dsTaxDetail.Tables[0].Rows[i]["Price"].ToString())) *
                                Convert.ToDouble(CustomCDBL(dsTaxDetail.Tables[0].Rows[i]["Qty"].ToString() == "" ? 0 : double.Parse(dsTaxDetail.Tables[0].Rows[i]["Qty"].ToString())))) *
                                Convert.ToDouble(Dep) / 100);
                            rowCusTaxDT["Dep"] = Dep;
                            ...
                            rowCusTaxDT["ExTaxAmount"] = Math.Round((Convert.ToDecimal(depamt) * Convert.ToDecimal(CustomCDBL( dsTaxDetail.Tables[0].Rows[i]["ExTaxAmount"].ToString()))) / 100, 2);
        ...
        private void SaveItemSalesTax(string InvoiceNumber, string InvNo = "", SqlTransaction Trans = null)
        {
            try {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (myFunc.ExecuteQuery(sql, Trans))
                    {
        ...
        private void SaveItemSalesTaxDep(string InvoiceNumber, string InvNo = "" ,SqlTransaction Trans=null)
        {
            try {
                ...
                {  
                    ...
                    foreach (GridViewRow row in InsCusGVD.Rows)
                    
                    {

                        string ityp = row.Cells[7].Text.ToString();
                        double gstPerc = CustomCDBL(row.Cells[10].Text);
                        double fgstPerc = CustomCDBL(row.Cells[12].Text);
                        double exTaxPerc = CustomCDBL(row.Cells[14].Text);
                            double gstAmt = CustomCDBL(row.Cells[11].Text);
                            double fgstAmt = CustomCDBL(row.Cells[13].Text);
                            double exTaxAmt = CustomCDBL(row.Cells[15].Text);
                            double dep = CustomCDBL(row.Cells[8].Text); ;
                        double depAmt = CustomCDBL(row.Cells[9].Text); ;

                        string itmCode = row.Cells[0].Text;
                            double taxAmount = CustomCDBL(row.Cells[6].Text) ;
                        string jcno =txtJobCardCode.Text;
        ...
        private void SaveItemSalesTaxIns(string InvoiceNumber, string InvNo = "",SqlTransaction Trans=null)
        {
            try {
                double totTax = 0;
                // Calculate PartItems Tax
        ...
        protected void gvTax_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow && (e.Row.RowState == DataControlRowState.Alternate || e.Row.RowState == DataControlRowState.Normal))
            {
                if (txtJobCardType.Text == "INSURANCE")
                {
                    ...
                    totalAmtwithTax = totalAmtwithTax + double.Parse(e.Row.Cells[9].Text.Replace("&nbsp;", "").Trim() == "" ? "0" : e.Row.Cells[9].Text.Replace("&nbsp;", "").Trim());
        ...
        public static double CustomCDBLTo0(object txt)
        {
            if (object.ReferenceEquals(txt, DBNull.Value)) return 0;
            else if (txt == null) return 0;
            else if (string.IsNullOrEmpty(txt.ToString().Trim())) return 0;
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                sysfun.UserMsg(lblMsg, Color.Red, "Can't Edit or Delete the Voucher '" + txtVoucherNo.Text + "' is already Posted");
                return;
            }
            ...
            try
            {
                myFunc.UpdateJV(Session["DealerCode"].ToString(), txtVoucherNo.Text);
                if (ObjTrans.BeginTransaction(ref Trans) == true)
                {
        ...
        {
            if (!sec.UserRight("2502", "002"))
            {
                Response.Redirect("~/Test.aspx");
            }
            if (ddlInvoiceNo.SelectedIndex != 0)
            {
                if (!sec.UserRight("2502", "003"))
                {
                    Response.Redirect("~/Test.aspx");
                }
            }
            ...
            else
            {
                ...
                if (txtJobCardType.Text.Trim() != "INSURANCE")
                {
                    ...
                    {
                        if (ddlReciptMode.SelectedValue=="Auto")
                        {
            ...
            if (ViewState["InvoiceNo"] != null)
            {
                ...
                if (PostServiceFollowUp(ViewState["InvoiceNo"].ToString(), "InvoiceCreated") == false)
                {
        ...
        protected void btnDeleteInv_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (ObjTrans.BeginTransaction(ref Trans) == true)
                {
                    ...
                    if (Create_NDM(txtJobCardCode.Text, "InvoiceDeleted") == false)
                    {
        ...
        protected void txtDiscLabor_TextChanged(object sender, EventArgs e)
        {
            double JobsSublet= CustomCDBL(txttotJobSublet.Text);
            double PST = CustomCDBL(txtPSTPercent.Text);
            double LabourDiscAmt;
            double AfterDiscountedAmount= CustomCDBL(txtDiscLabor.Text);
            double PSTAmount = CustomCDBL(txtPSTAmount.Text);
            double TotalCusNetAmt = CustomCDBL(txtTotalAmtCustomer.Text);
            LabourDiscAmt = CustomCDBL((JobsSublet - AfterDiscountedAmount) * PST / 100);
            txtPSTAmount.Text = LabourDiscAmt.ToString();
            CalculateSubTotal();
            txtTotalAmtCustomer.Text = ((TotalCusNetAmt - PSTAmount)-(AfterDiscountedAmount)+LabourDiscAmt).ToString();


        }
        public void CalculateSubTotal()
        {
            double JobsSublet = CustomCDBL(txttotJobSublet.Text) - CustomCDBL(txtDiscLabor.Text);
            double PartsLub = CustomCDBL(txttotPartsLub.Text) - CustomCDBL(txtDiscountParts.Text);
            double GST = CustomCDBL(txtGSTPercent.Text);
            double PST = CustomCDBL(txtPSTPercent.Text);
            double LabourDiscAmt,PartsDiscAmt;
            LabourDiscAmt = CustomCDBL((JobsSublet) * PST / 100);
            PartsDiscAmt = CustomCDBL((PartsLub) * GST / 100);
            double GSTAmount = CustomCDBL(txtTaxAmount.Text);
            double PSTAmount = CustomCDBL(txtPSTAmount.Text);
            double SubTotal = JobsSublet+PartsLub+LabourDiscAmt+PartsDiscAmt+CustomCDBL(txtBoutTotal.Text);
            txtSubTotal.Text = SubTotal.ToString();

        }
    

        protected void txtDiscountParts_TextChanged(object sender, EventArgs e)
        {
            double JobsSublet = CustomCDBL(txttotJobSublet.Text) - CustomCDBL(txtDiscLabor.Text);
            double PartsLub = CustomCDBL(txttotPartsLub.Text) - CustomCDBL(txtDiscountParts.Text);
            double TotalCusNetAmt = CustomCDBL(txtTotalAmtCustomer.Text);
            ...
            PartsDiscAmt = CustomCDBL((PartsLub) * GST / 100);
            double GSTAmount = CustomCDBL(txtTaxAmount.Text);
            txtTaxAmount.Text = PartsDiscAmt.ToString();
            double PSTAmount = CustomCDBL(txtPSTAmount.Text);
            double SubTotal = JobsSublet + PartsLub + LabourDiscAmt + PartsDiscAmt + CustomCDBL(txtBoutTotal.Text);
            txtSubTotal.Text = SubTotal.ToString();
            txtTotalAmtCustomer.Text = CustomCDBL((TotalCusNetAmt - PSTAmount-GSTAmount) - CustomCDBL(txtDiscLabor.Text)- CustomCDBL(txtDiscountParts.Text) +PartsDiscAmt+ LabourDiscAmt).ToString();
        }
            ...
            if (e.Row.RowType == DataControlRowType.DataRow && (e.Row.RowState == DataControlRowState.Alternate || e.Row.RowState == DataControlRowState.Normal))
            {
                if (txtJobCardType.Text == "INSURANCE")
                {
                    totalCustax = totalCustax + double.Parse(e.Row.Cells[11].Text.Replace("&nbsp;", "").Trim() == "" ? "0" : e.Row.Cells[11].Text.Replace("&nbsp;", "").Trim()) +
                                       double.Parse(e.Row.Cells[13].Text.Replace("&nbsp;", "").Trim() == "" ? "0" : e.Row.Cells[13].Text.Replace("&nbsp;", "").Trim()) +
                                       double.Parse(e.Row.Cells[15].Text.Replace("&nbsp;", "").Trim() == "" ? "0" : e.Row.Cells[15].Text.Replace("&nbsp;", "").Trim());
                    totalCusAmtwithTax = totalCusAmtwithTax + double.Parse(e.Row.Cells[9].Text.Replace("&nbsp;", "").Trim() == "" ? "0" : e.Row.Cells[9].Text.Replace("&nbsp;", "").Trim()) ;
                }
                else
                {
       ...
       #region
        protected void btnGatePass_Click(object sender, EventArgs e)
        {
          ...
          // /*2*/ new SqlParameter("@GatePassDate",SqlDbType.DateTime),     
           ...
           /*4*/ new SqlParameter("@InvoiceNo",SqlDbType.VarChar,8),
                ...
                PostServiceFollowUp(oldgatepass, "GatePassGenerated");
        ...
        protected void btnPerInv_Click(object sender, EventArgs e)
        {
            ...
            if (RadioButton3.Checked)
            {
                if (txtJobCardType.Text == "INSURANCE")
                {
                    ...
                    {
                        Load_CombineInvoiceRptPerforma("~\\Modules\\Service\\ServiceReports\\Ins_CombineReportPerfor.rpt");
                    }
                }
                else
                {
        ...
        protected void btnPrintInv_Click(object sender, EventArgs e)
        {
            ...
            if (RadioButton3.Checked)
            {
                ...
                {
                    Load_CombineInvoiceRpt("~\\Modules\\Service\\ServiceReports\\CombineReport.rpt");
                }
        ...
        private void Load_CombineInvoiceRpt(string rpt)
        {
            ...
            //RD.DataDefinition.FormulaFields["DealershipName"].Text = "'Authorised " + Session["ParentDesc"].ToString() + " Dealership'";
            RD.DataDefinition.FormulaFields["QRCode"].Text = "'" + Server.MapPath("~") + "/Images/QrCode.jpg'";
             RD.DataDefinition.FormulaFields["Whatsapp"].Text = "'" + Server.MapPath("~") + Session["Whatsapp"] + "'";
            RD.DataDefinition.FormulaFields["WhatsappNo"].Text = "'" + Session["WhatsappNo"].ToString() + "'";
            RD.DataDefinition.FormulaFields["ComplainNo"].Text = "'" + Session["ComplainNo"].ToString() + "'";
        ...
        private void Load_CombineInvoiceRptPerforma(string rpt)
        {
            ...
            //RD.Load(Server.MapPath("~\\Reports\\GRN_SSTInvoiceReport.rpt"));
            RD.Load(Server.MapPath(rpt));
            RD.DataDefinition.FormulaFields["Customer"].Text = "'" + dsJobCardMaster.Rows[0]["Customer"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["KM"].Text = "'" + dsJobCardMaster.Rows[0]["KM"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["Address"].Text = "'" + dsJobCardMaster.Rows[0]["Address1"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["Phone"].Text = "'" + dsJobCardMaster.Rows[0]["Phone"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["Cell"].Text = "'" + dsJobCardMaster.Rows[0]["CellNo"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["NIC"].Text = "'" + dsJobCardMaster.Rows[0]["NIC"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["NTNno"].Text = "'" + dsJobCardMaster.Rows[0]["NTNno"].ToString() + "'";
            RD.DataDefinition.FormulaFields["ContPersonName"].Text = "'"+ dsJobCardMaster.Rows[0]["ContPersonName"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["ContPersonCell"].Text = "'" + dsJobCardMaster.Rows[0]["ContPersonCell"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["EngineNo"].Text =    "'"+txtEngineNo.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["ChassisNo"].Text = "'" + txtChassisNo.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["EndUser"].Text =     "'"+txtUserName.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["RegNo"].Text =       "'"+txtRegNo.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["ProdCode"].Text =    "'"+txtProductCode.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["ProdName"].Text =    "'"+txtProductName.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["JobCardCode"].Text = "'"+txtJobCardCode.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["PaymentType"].Text = "'"+txtBillPayMode.Text.ToUpper() + "'";
            RD.DataDefinition.FormulaFields["PSTAmountPerforma"].Text = "'" + CustomCDBL(txtPSTAmount.Text).ToString().ToUpper()+"'";
            RD.DataDefinition.FormulaFields["GSTAmountPerforma"].Text = "'" + CustomCDBL(txtTaxAmount.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["CusGSTAmountPerforma"].Text = "'" + CustomCDBL(lblCusTaxAmt.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DiscountLabor"].Text ="'"+ CustomCDBL(txtDiscLabor.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DiscountPart"].Text = "'" + CustomCDBL(txtDiscountParts.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DiscountPartPer"].Text = "'" + CustomCDBL(txtDiscLaborPercent.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DiscountPartPer"].Text = "'" + CustomCDBL(txtDiscPercentPart.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["PSTPer"].Text = "'" + CustomCDBL(txtPSTPercent.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["GSTPer"].Text = "'" + CustomCDBL(txtGSTPercent.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["GrossAmountPerforma"].Text = "'" + CustomCDBL(txtGrossAmount.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["SubTotalPerforma"].Text = "'" + CustomCDBL(txtSubTotal.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["TotalNetPerforma"].Text = "'" + CustomCDBL(txtTotalAmtCustomer.Text).ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["TotalInsuranceAmt"].Text = "'" + CustomCDBL(txtTotAmtInsComp.Text ).ToString()+ "'";
            RD.DataDefinition.FormulaFields["TotalNetPerforma"].Text = "'" + CustomCDBL(txtTotalAmtCustomer.Text).ToString() + "'";
            RD.DataDefinition.FormulaFields["DealerDesc"].Text = "'" + Session["DealerDesc"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DealerAddress"].Text = "'" + Session["DealerAddress"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DealerPhone"].Text = "'" + Session["DealerPhone"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DealerEmail"].Text = "'" + Session["DealerEmail"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["DealerFax"].Text = "'" + Session["DealerFax"].ToString().ToUpper() + "'";
            RD.DataDefinition.FormulaFields["ReportTitle"].Text = "'COMBINE INVOICE'".ToUpper();
        ...
        private void Load_GSTInvoiceRpt(string rpt)
        {
            ...
            string strJCCode = ds.Tables[0].Rows[0]["JobCardCode"].ToString();
            string InvType = ds.Tables[0].Rows[0]["InvType"].ToString().Trim();
            if (txtJobCardType.Text == "INSURANCE")
            {
                if (ds.Tables[0].Rows[0]["InvType"].ToString().Trim() == "Ins")
                {
                    ds = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(cCon, CommandType.Text, "sp_InsDepCust_CombineInvoiceParts_Select'" + this.Session["DealerCode"].ToString() + "','" + strJCCode + "','" + ds.Tables[0].Rows[0]["InvType"].ToString().Trim() + "'"); //
                }
                else
                {
                    ds = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(cCon, CommandType.Text, "sp_InsDepCust_CombineInvoiceParts_Select'" + this.Session["DealerCode"].ToString() + "','" + strJCCode + "','" + "Dep" + "'"); //
                }
