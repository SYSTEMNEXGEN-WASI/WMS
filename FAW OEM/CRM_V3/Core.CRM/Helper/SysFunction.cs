...
public class SysFunction
{
    ...
    public string GetNewMaxIDAppend(string sTableName, string sColumn, int NoOfChar, string DealerCode, string Append)
    {
        //#GET MAX ID FROM TABLE
        string sQuery = "SELECT MAX(" + sColumn + ") MAXID FROM " + sTableName + " WHERE DealerCode='" + DealerCode + "'" + Append;
    ...
    {
        string usrname = "";
        DataTable dt = GetData("select * from SecurityUser where ID='" + SessionUserID + "' and DealerCode='"+DealerCode+"'");
        if (dt.Rows.Count > 0)
        {
    ...
    public string GetNewMaxIDWitOutDealerCode(string sTableName, string sColumn, int NoOfChar, string Append)
    {
        //#GET MAX ID FROM TABLE
        string sQuery;
        if (Append=="" || Append == null)
        {
             sQuery = "SELECT MAX(" + sColumn + ") MAXID FROM " + sTableName;
        }
        else
        {
            sQuery = "SELECT MAX(" + sColumn + ") MAXID FROM " + sTableName+" "+Append ;
        }
    ...
    //ISEXIST with Out Dealer
    #region
    public bool IsExistwithoutDealer(string strColumnName, string strValue, string strTableName, string Append = "")
    {
    ...
    public bool IsExist(string strColumnName, string strValue, string strTableName, string DealerCode, string Append = "")
	{
		SqlDataReader dr = null;
		string strSql = null;
		strSql = "Select [" + strColumnName + "] from [" + strTableName + "] Where [" + strColumnName + "] = '" + strValue + "' AND DealerCode IN ('" + DealerCode + "','COMON','AAAAA') " + Append;
		try
		{
			//if (!dr.IsClosed)
			//{
